...
def test_convert(_rule_dict, _rule_strings, _data):
    ...
    r = ConvertRuleDictsToRuleStrings(rule_dicts=rule_dicts)
...
def test_convert_rule(_rule_dict, _rule_strings, _data):
    ...
    rule_dict = _rule_dict
    for as_numpy, expected_rule_string in expected_rule_strings.items():
        r = ConvertRuleDictsToRuleStrings(rule_dicts={})
...
def test_parse_value_lambda():
    condition_dict1 = {'field': 'A',
                       'operator': 'greater_or_equal',
                       'value': 60.0}
    condition_dict2 = {'field': 'A',
                       'operator': 'less',
                       'value': 100.0}
    expected_results = {
    ...
    for with_kwargs in [True, False]:
        r = ConvertRuleDictsToRuleStrings(rule_dicts={})
        value = r._parse_value_lambda(condition_dict=condition_dict1,
                                      field='A',
                                      with_kwargs=with_kwargs)
        assert value == expected_results[with_kwargs][0]
        value = r._parse_value_lambda(condition_dict=condition_dict2,
                                      field='A',
                                      with_kwargs=with_kwargs)
