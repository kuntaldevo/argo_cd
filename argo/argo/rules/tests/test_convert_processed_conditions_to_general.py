...
import pandas as pd
import numpy as np
from rules.convert_processed_conditions_to_general import \
    ConvertProcessedConditionsToGeneral, ReturnMappings
...
class TestConvertProcessedConditionsToGeneral:
    ...
    def test_recurse_create_condition_replacement_dict(self, _data, _instantiate_class):
        ...
        c._recurse_create_condition_replacement_dict(
            rule_string=rule_strings['combined_rule_5'], X=X_processed)
    ...
    def test_add_null_condition_to_imputed_numeric_rule(self, _data, _instantiate_class,
                                                        _expected_general_rule_strings):
        rule_components = {
            'nan_low': ('A', '>=', '0'),
            'nan_middle': ('A', '>=', '-1'),
            'nan_high': ('A', '<=', '0'),
        ...
        _, X_processed, imputed_values, _ = _data
        c = _instantiate_class
        ...
        for rule_name, rule_comp in rule_components.items():
            cleaned_string = c.add_null_condition_to_imputed_numeric_rule(
    ...
    def test_convert_ohe_condition_to_general(self, _data, _instantiate_class):
        ...
        cleaned_strings = {}
        for rule_name, rule_comp in rule_components.items():
            cleaned_string = c.convert_ohe_condition_to_general(
...
class TestReturnMappings:
    ...
    def test_return_ohe_categories_mapping(self):
        ...
        rm = ReturnMappings()
        ohe_mapping = rm.return_ohe_categories_mapping(pre_ohe_cols=pre_ohe_cols,
                                                       post_ohe_cols=post_ohe_cols,
